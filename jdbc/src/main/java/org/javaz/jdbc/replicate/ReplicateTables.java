...
*/
	...
	public void invoke(Object parameters) throws InterruptedException
	{
		if(verbose)
			...
			for (Iterator<HashMap<String, String>> HashMapIterator = tables.iterator(); HashMapIterator.hasNext();)
			{
				try
				{
					...
					if (b)
					{
						...
						{
							meta.put(setMetaData.getColumnLabel(i), setMetaData.getColumnClassName(i));
						}
					...
					if (b)
					{
						...
						for (int i = 1; i <= setMetaData.getColumnCount(); i++)
						{
							meta2.put(setMetaData.getColumnLabel(i), setMetaData.getColumnClassName(i));
						}
						while (resultSet.next())
						{
							HashMap results = new HashMap();
